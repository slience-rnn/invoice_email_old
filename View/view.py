...
def view(google:GoogleClass):
    ...
    return_err1_arr = []
    return_err2_arr = []
    temp = ['template1','template2','template3','Receipt_Remind','Receipt_Remind-IRS','Invoice_Remind_202510','Invoice_Form_202601']
    send_failed = []
    ...
    client = f'CLIENT!A2:AG{num-1}' #'CLIENT!A2:AG200'
    summSheet =  f'SummSheet!A{start}:AU{num}' #'SummSheet!A3:AU201'
    ...
    mc_header_region = 'Progress Tracker(M)!A2:F2'
    mc_pt = f'Progress Tracker(M)!A{start}:F{num}'

    # 读取所有表的表头
    header_region_name = [client_header_region,summSheet_header_region]
    header = google.read_sheet_batchGet(header_region_name, tracker_sheet_id)
    ...
    sheet_data = google.read_sheet_batchGet(range_name,tracker_sheet_id)
    ...
    receipt_csv_path = Path('./doc/receipt_generate_sended.csv')
    ...
    if st.button('🧾 生成 支付单 Invoice'): # 开始执行 
        ...
        if no_imi == [] and no_email == []:
            ...
            for line in range(summSheet.shape[0]):
                ...
                #print(f'fee_data中total amount eur {fee_data['TOTAL AMOUNT €']}')
                #print(f'{fee_data}')
                ...
                print(f'write Done {file_meta['invoiceno']}')
                # st.write(result)
    ...
    if st.button('📤 查询 Drive 并发送INV至客户邮箱'):
        ...
        # client_data = deal_sheet_data(sheet_data[0].get('values',''),client_header,name = 'client')
        # summSheet = deal_sheet_data(sheet_data[1].get('values',''),summSheet_header,name = 'summSheet')
        # tax_rep_fee = deal_sheet_data(sheet_data[2].get('values',''),name='tax_rep_fee')  
        #file_meta = generate_invoice(google,sheet_data,select_year)

        
        if no_imi == [] and no_email == []:
            st.info('开始执行......')
            send_invoice(google,summSheet,client_data,select_year,select_templated)
